...
import { CheckCircle2, XCircle, Clock, Monitor, ChevronRight, LayoutGrid, List } from 'lucide-react'
...
import { attendanceRecords, type AttendanceStatus, type AttendanceMethod } from '@/lib/mock-data'
...
function getRandomStatus(day: number) {
  if (day > 7) return 'H'
  const opts = ['P', 'P', 'P', 'P', 'A', 'L', 'W']
  return opts[day % opts.length]
}

const summaryCards = [
  { label: 'Present', value: 201, icon: CheckCircle2, gradient: 'from-blue-500 to-indigo-600', change: 'Today' },
  { label: 'Absent', value: 29, icon: XCircle, gradient: 'from-rose-500 to-pink-600', change: 'Today' },
  { label: 'Late', value: 18, icon: Clock, gradient: 'from-amber-400 to-orange-500', change: 'Today' },
  { label: 'WFH', value: 14, icon: Monitor, gradient: 'from-indigo-500 to-violet-600', change: 'Today' },
]
              ...
              {view === 'grid' ? (
                ...
                >
                  {attendanceRecords.map((rec, i) => (
                    <div key={rec.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all overflow-hidden group">
                      {/* Top color band based on status */}
                      ...
                      <TableBody>
                        {attendanceRecords.map((rec, i) => (
                          ...
                          >
                            ...
                            <TableCell className="text-[12px] text-slate-700 font-medium">
          ...
          <TabsContent value="monthly">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm shadow-slate-100">
              <CardHeader className="pb-3">
                <CardTitle className="text-[15px] font-semibold text-slate-800">April 2026 — Arjun Sharma</CardTitle>
              ...
              <CardContent>
                ...
                <div className="grid grid-cols-7 gap-1.5">
                  ...
                  {daysInMonth.map((day) => {
                    const statusKey = getRandomStatus(day)
                    return (
                      <motion.div
                        key={day}
                        ...
                        className={`aspect-square rounded-xl flex flex-col items-center justify-center border text-[11px] font-semibold cursor-pointer hover:opacity-80 transition-opacity ${calendarColors[statusKey]}`}
                      >
                        <span>{day}</span>
                        <span className="text-[8px] font-medium opacity-80">{statusKey === 'H' ? '' : statusKey}</span>
                      </motion.div>
                    )
                  })}
                </div>
